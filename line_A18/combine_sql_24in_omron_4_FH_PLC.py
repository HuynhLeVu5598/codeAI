...
def program_camera1_FH(model,size,conf):
    read_1900 = fins_instance.memory_area_read(FinsPLCMemoryAreas().DATA_MEMORY_WORD,b'\x0F\xA0\x00') # doc thanh ghi 1900
    #print(read_4000)
    if read_1900 == b'\xc0\x00\x02\x00\x19\x00\x00\x01\x00`\x01\x01\x00\x00\x00\x01':  # gia tri 1
        directory1 = 'C:/FH/camera1/'
        if os.listdir(directory1) == []:
            ...
            for filename1 in glob.glob('C:/FH/camera1/*'):
                for path1 in glob.glob(filename1 + '/*'):
                    ...
                    if name == 'Input0_Camera0.jpg':
                        ...
                        # ghi vao D1910 gia tri 0
                        fins_instance.memory_area_write(FinsPLCMemoryAreas().DATA_MEMORY_WORD,b'\x0F\xA0\x00',b'\x00\x01',1)
...
def program_camera2_FH(model,size,conf):
    read_1902 = fins_instance.memory_area_read(FinsPLCMemoryAreas().DATA_MEMORY_WORD,b'\x0F\xA2\x00') # doc thanh ghi 1902
    if read_1902 ==b'\xc0\x00\x02\x00\x19\x00\x00\x01\x00`\x01\x01\x00\x00\x00\x01':  # gia tri 1
        directory2 = 'C:/FH/camera2/'
        if os.listdir(directory2) == []:
...
def program_camera3_FH(model,size,conf):
    read_1904 = fins_instance.memory_area_read(FinsPLCMemoryAreas().DATA_MEMORY_WORD,b'\x0F\xA4\x00') # doc thanh ghi 1904
    if read_1904 == b'\xc0\x00\x02\x00\x19\x00\x00\x01\x00`\x01\x01\x00\x00\x00\x01':  # gia tri 1
        directory3 = 'C:/FH/camera3/'
        if os.listdir(directory3) == []:
...
def program_camera4_FH(model,size,conf):
    read_1906 = fins_instance.memory_area_read(FinsPLCMemoryAreas().DATA_MEMORY_WORD,b'\x0F\xA6\x00') # doc thanh ghi 1906
    if read_1906 == b'\xc0\x00\x02\x00\x19\x00\x00\x01\x00`\x01\x01\x00\x00\x00\x01':  # gia tri 1
        directory4 = 'C:/FH/camera4/'
        if os.listdir(directory4) == []:
